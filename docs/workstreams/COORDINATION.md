...
Protocol version: `3`
Last coordinator review: `2026-07-15`
...
|---|---|---|---|---|
| Tetris T3 | historical implementation + QA | `E:\Proj\Game-1` / `codex/tetris` | T3 campaign and Mineral Shelf V1 are integrated and pushed in `4c85828`; no product slice is open. | `ACK-TETRIS-T3-001`: reviewed final accepted chain `6fb1728 -> fdd1ffb -> 4c85828`, including final live browser check. The historical core/frontend logs are closed by the 2026-07-15 coordinator reconciliation. |
...
## Active roster and tool allocation

The active roster is intentionally minimal.

| Role | Status | Exact responsibility |
| --- | --- | --- |
| Coordinator | active | Verifies reports and Git evidence, owns this register and root changelog, integrates only accepted work, and opens the next bounded slice immediately after a valid terminal acknowledgement. |
| Tetris | closed | T3 is published. No implementation, design, or QA worker is active unless a future user-approved enhancement opens a new slice. |
| Temple A6 reviewer | active, read-only | Validates the A6 preflight contract only and writes the two listed review documents. |

Use the smallest toolchain that matches the next slice: `frontend-design`,
`develop-web-game`, Playwright/webapp testing, and Context7 for a future Tetris surface
change; `blender-webgl-asset-proof`, Context7, and clean-room image generation only
when Temple reaches a separately authorized proof/art slice; browser QA uses Playwright
or Chrome plus checked-in candidate evidence. Context7 is available; credentials and
configuration values must never be copied into reports or repository files. No further
plugin or skill installation is justified by the current state.

## Feedback recovery rule

An external task is not considered active merely because it appears in the Codex UI.
The coordinator records `ACK-...-STARTED` only after issuing an exact scope with a
named log path. At terminal state it reads the thread once and verifies the durable
`THREAD_LOG.md`, SHA/path range, and evidence. If the thread API is unavailable, one
log-and-Git fallback is authoritative. A missing required log immediately changes the
register state to blocked and the associated heartbeat is deleted; it is never treated
as progress. The next writer starts only after the previous terminal report is
acknowledged, preventing duplicate or silently overlapping work.
